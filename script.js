...
continue_btn.onclick = ()=>{
    ...
    startTimerLine(60); //calling startTimerLine function
}
...
function startTimerLine(time) {
    widthValue = 0; // reset widthValue to 0
    var maxWidth = 549; // maximum width of the progress bar
    var increment = maxWidth / (time * 1000 / 29); // calculate the increment value

    counterLine = setInterval(timer, 29);
    function timer() {
        widthValue += increment; // increment widthValue
        time_line.style.width = widthValue + "px"; // increase width of time_line with px by widthValue
        if (widthValue > maxWidth) { // if widthValue is greater than maxWidth
            clearInterval(counterLine); // clear counterLine
        }
    }
}
...
next_btn.onclick = () => {
    if (que_count < questions.length - 1) { // if question count is less than total question length
        ...
        startTimerLine(60); // start progress bar for 60 seconds
